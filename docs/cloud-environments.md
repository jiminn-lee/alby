...
The Edge Function requires an Alby user JWT. Search is read-only. Materialization uses the function's service-role client to call the protected catalog function. MusicBrainz metadata IDs are stored separately from Alby's internal album IDs, and Cover Art Archive provides remote artwork URLs.
