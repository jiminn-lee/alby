...
add constraint activity_source_matches_type check (
  ...
  or (activity_type = 'listen_later_added' and rating_id is null)
);

create unique index activity_listen_later_actor_album_idx
on public.activity_events (actor_id, album_id)
...
begin
  ...
  on conflict (actor_id, album_id) where activity_type = 'listen_later_added'
  do update set listen_later_item_id = excluded.listen_later_item_id;
  return new;
end;
$$;
