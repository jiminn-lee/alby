...
export default function SignInScreen() {
  const { signInDemo, signInWithApple, signInWithGoogle } = useAuth();
    ...
    <SafeAreaView style={styles.safeArea}>
      <View style={styles.content}>
        ...
        <View style={styles.actions}>
          {Platform.OS === 'ios' && <AlbyButton disabled={busy} icon={AppleLogoIcon} label="Continue with Apple" onPress={() => run(signInWithApple)} />}
          <AlbyButton disabled={busy} icon={GoogleLogoIcon} label="Continue with Google" onPress={() => run(signInWithGoogle)} variant="secondary" />
